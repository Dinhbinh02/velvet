...
import { X, Type, Columns, RotateCcw, AlignLeft, AlignJustify, Palette, Upload, Trash2, Keyboard, Bot, Highlighter } from 'lucide-react';
...
export const TypographyDrawer: React.FC<TypographyDrawerProps> = ({
  ...
  const defaultFontFamilies = [
    { id: 'Literata, Georgia, serif', label: 'Literata (Book Serif)' },
    { id: 'Inter, -apple-system, sans-serif', label: 'Inter (Clean Sans)' },
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-84 bg-[var(--bg-surface)]/95 backdrop-blur-2xl border-l border-[var(--border-color)] shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-200 select-none">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-3 gap-2">
            {themes.map((t) => (
              ...
              >
                <div className="w-4 h-4 rounded-full border border-black/10 flex items-center justify-center text-[9px] font-bold">
                  {settings.theme === t.id ? '✓' : ''}
                </div>
          ...
          <div className="grid grid-cols-1 gap-1.5">
                ...
                {Array.from(new Set(customFonts.map((f) => f.name))).map((familyName) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-full p-2.5 rounded-xl text-left text-xs transition-all flex items-center justify-between cursor-pointer border group ${
                        isSelected
                      ...
                      <div className="flex items-center gap-2 min-w-0 pr-2">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {familyVariants.map((v) => {
                            const label = v.fileName.toLowerCase().includes('bold')
                              ? 'B'
                              : v.fileName.toLowerCase().includes('italic')
                              ? 'I'
                              : 'R';
                            return (
                              <span
                                key={v.id}
                                className="text-[9px] uppercase px-1 py-0.2 rounded bg-[var(--bg-surface)] text-[var(--text-muted)] border border-[var(--border-color)]"
                                title={v.fileName}
                              >
                                {label}
                              </span>
            ...
            {defaultFontFamilies.map((font) => (
              <button
                ...
                className={`w-full p-2.5 rounded-xl text-left text-xs transition-all flex items-center justify-between cursor-pointer border ${
        ...
        <div className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)] space-y-2.5">
          <span className="text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] flex items-center gap-1.5">
          ...
          <p className="text-[10px] text-[var(--text-muted)] leading-relaxed">
            Used for instant vocabulary lookup, definitions, and IPA pronunciation.
          </p>
        </div>
      </div>

      {/* Reset Defaults */}
      <div className="p-4 border-t border-[var(--border-color)] bg-[var(--bg-surface)] shrink-0">
        <button
          onClick={() => onUpdate(DEFAULT_SETTINGS)}
          className="w-full py-2.5 rounded-xl border border-[var(--border-color)] hover:bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-xs font-medium transition-all flex items-center justify-center gap-1.5 cursor-pointer"
        >
